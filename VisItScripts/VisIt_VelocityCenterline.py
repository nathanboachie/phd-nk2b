...
for domain_folder,name in zip(domain_folders,names):
  ...
  write_folder=f'velocity_{domain_folder}'
  ...
  for ts in range(0,nt):
      ...
      Velocity=Data[1::2]
      Position=Data[::2]
      np.savetxt(f"/work/e01/e01/exy214/rdemic-dir/RDEMIC/{domain_folder}/domain/{write_folder}/velocity{ts:04d}.txt", Velocity)
      if ts == 0:
